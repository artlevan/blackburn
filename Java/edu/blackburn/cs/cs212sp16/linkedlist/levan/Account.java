 ...
 * that have an integer id. A positive integer id. Could be used as a base class
 * for tons of kinds of accounts. New Account objects cannot be created
...
public class Account {
    ...
    /**
     * The maximum number of Account objects that can be created; set to 1
     * million.
     */
    ...
    /**
     * Compares two objects. If the object passed in is of type Account and its
     * id is equal to this object's id, the two are equal, and the method
    ...
    /**
     * Randomly generates a new Account object, adds it to the EXTANT List, and
     * returns that Account object 2/3 of the time, or if MAXSIZE Account
     * objects have been created. Otherwise returns a random existing Account
     * object from the EXTANT List. Will never create duplicates.
     *
    ...
    public static Account getRandom() {
        ...
        if ((makeNew == 0 && EXTANT.size() > 0) || EXTANT.size() >= MAXSIZE) {
            ...
            if (LOG) {
                System.err.println("Existing - Id: " + a.getId() + " Element: " + tmp);
            }
            return a;
            ...
            while (true) {
                ...
                if (LOG) {
                    System.err.println("New: " + tmp + " Total: " + EXTANT.size());
                }
                return a;
            }
        } else {
    ...
    /**
     * Turn on logging (defaulted to off) DON'T USE THIS
     *
